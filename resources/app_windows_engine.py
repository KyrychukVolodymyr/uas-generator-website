...
def get_engine_resource_dir():
    """
    Normal Python mode: folder containing app_windows_engine.py.
    Packaged EXE mode: launcher sets UAS_RESOURCES_DIR to bundled _internal folder.
    """
    import os
    import sys
    ...
    env_dir = os.environ.get("UAS_RESOURCES_DIR", "").strip()
    if env_dir:
...
ENGINE_RESOURCE_DIR = get_engine_resource_dir()
RESOURCE_DIR = ENGINE_RESOURCE_DIR
BASE_DIR = ENGINE_RESOURCE_DIR


def get_resource_path(name):
    return str(ENGINE_RESOURCE_DIR / name)
...
APP_DIR = ENGINE_RESOURCE_DIR

def get_runtime_dir():
    if os.name == "nt":
        base = os.environ.get("APPDATA") or str(Path.home() / "AppData" / "Roaming")
        return Path(base) / "UASGenerator2026V2"
    return Path.home() / "Library" / "Application Support" / "UASGenerator2026V2"
...
def parse_generated_paths(stdout_text: str):
    ...
    for raw_line in str(stdout_text or "").splitlines():
        ...
        if not (low.endswith(".xlsx") or low.endswith(".pdf")):
            continue

        # Windows PowerShell/logs may preserve paths exactly, but we also normalize safely.
        candidate = Path(line)
        ...
        # Sometimes paths may be copied with extra whitespace or hidden characters.
        ...
        # Last-resort fallback: if the exact file was generated under Documents,
        # search by filename in the standard generated output root.
        try:
            filename = Path(cleaned).name
            if filename:
                doc_root = Path.home() / "Documents" / "Anthem Blue Cross Blue Shield Assessment Documentation"
                if doc_root.exists():
                    matches = list(doc_root.rglob(filename))
                    for m in matches:
                        if m.exists() and m not in paths:
                            paths.append(m)
        except Exception:
            pass
...
def _pdf_form_value_to_text(value):
    if value is None:
        return ""
    try:
        text = str(value)
...
def flatten_pdf_form_visible(pdf_path: Path):
    """
    Adobe-safe FRA flattening for Windows.
    Stronger centering for right-side numeric points fields.
    """
    ...
    try:
        ...
        for name, data in fields.items():
            ...
            try:
                ...
                val = None
            txt = _pdf_form_value_to_text(val)
            if txt:
                field_values[str(name)] = txt
    except Exception:
        field_values = {}
    ...
    for page in reader.pages:
        ...
        if annots:
            ...
            for annot_ref in list(annots_obj):
                ...
                field_name = _pdf_form_value_to_text(annot.get("/T"))
                field_type = str(annot.get("/FT", ""))
                annot_value = _pdf_form_value_to_text(annot.get("/V"))
                appearance_state = _pdf_form_value_to_text(annot.get("/AS"))

                box_w = max(1, x2 - x1)
                box_h = max(1, y2 - y1)

                if field_type == "/Btn":
                    value_text = field_values.get(field_name) or annot_value or appearance_state
                    if value_text and value_text.lower() not in {"off", "no", "false", "0"}:
                        c.setFont("Helvetica-Bold", 8)
                        c.drawString(x1 + 2, y1 + 2, "X")
                    continue

                value_text = field_values.get(field_name) or annot_value
                if not value_text:
                    continue
                ...
                max_chars = max(3, int(box_w / max(font_size * 0.48, 3)))
                if len(value_text) > max_chars:
                    value_text = value_text[:max_chars]

                y_text = y1 + max(2, (box_h - font_size) / 2) + 0.6
                c.setFont("Helvetica", font_size)

                compact_numeric = value_text.strip().replace(".", "", 1).isdigit()
                right_side_box = x1 >= (width * 0.78)
                small_numeric = compact_numeric and len(value_text.strip()) <= 3

                if small_numeric and right_side_box:
                    c.drawCentredString(x1 + (box_w / 2), y_text, value_text)
                else:
                    c.drawString(x1 + 2, y_text, value_text)
...
def _find_soffice_exe():
    ...
    env_soffice = os.environ.get("SOFFICE_PATH", "").strip()
    if env_soffice:
    ...
    for candidate in candidates:
        try:
            if candidate.lower() in {"soffice", "soffice.exe"}:
...
def _safe_local_temp_dir(prefix: str):
    import tempfile
...
def recalculate_workbook_formulas(xlsx_path: Path):
    """
    Keep formulas alive, but force a real spreadsheet engine to calculate and save cached values.
    This does NOT replace formulas with Python-calculated values.
    """
    ...
    temp_root = _safe_local_temp_dir("uas_lo_recalc_")
    ...
    local_xlsx = temp_in / xlsx_path.name
    ...
    result = subprocess.run(
        [
            ...
            "--nodefault",
            "--nofirststartwizard",
            f"-env:UserInstallation=file:///{str(temp_profile).replace(chr(92), '/')}",
            "--convert-to",
            "xlsx",
            "--outdir",
            str(temp_out),
    ...
    recalculated = temp_out / local_xlsx.name
    if result.returncode == 0 and recalculated.exists():
...
def export_excel_to_pdf(xlsx_path: Path):
    ...
    recalculate_workbook_formulas(xlsx_path)

    soffice = _find_soffice_exe()
    if not soffice:
        raise RuntimeError("Could not export Excel file to PDF on Windows. LibreOffice/soffice was not found.")

    temp_root = _safe_local_temp_dir("uas_lo_pdf_")
    temp_in = temp_root / "input"
    temp_out = temp_root / "output"
    temp_profile = temp_root / "profile"
    ...
    local_pdf = temp_out / local_xlsx.with_suffix(".pdf").name
    ...
    raise RuntimeError(
        "Could not export Excel file to PDF on Windows. "
        f"LibreOffice returned {result.returncode}. "
...
def create_cdpas_via_excel(template_path: Path, output_path: Path, assessment_date: str, rows, equipment, medicaid_id: str, member_name_value: str):
    print('V020Q_ENGINE_CREATE_CDPAS_ENTERED', flush=True)
    ...
    def writable_cell(cell_ref: str):
        target = ws[cell_ref]
    ...
    def safe_set(cell_ref: str, value):
        cell = writable_cell(cell_ref)
        cell.value = "" if value is None else value

    # Keep existing logo/header from the template.
    # Only force a readable text identity if the top area is blank.
    try:
        if not str(ws["A1"].value or "").strip():
            safe_set("A1", "Anthem Blue Cross Blue Shield")
    except Exception:
        pass

    # Patient/header data.
    for cell_ref, value in cell_values.items():
        safe_set(cell_ref, value)
...
def stamp_cdpas_pdf_logo_only(pdf_path: Path):
    print("V020D11_CDPAS_PDF_ONLY_LOGO_STAMP_ACTIVE", flush=True)

    pdf_path = Path(pdf_path)
    logo_path = Path(__file__).resolve().parent / "anthem_logo.jpeg"

    if not pdf_path.exists():
        raise RuntimeError(f"CDPAS PDF not found for logo stamping: {pdf_path}")

    if not logo_path.exists():
        raise RuntimeError(f"Anthem logo not found for PDF stamping: {logo_path}")
    ...
    # Correct placement: top-left header area, above patient information row.
    # This does not touch the Excel file.
    x = 18
    y = page_height - 45
    w = 135
    h = 28
    ...
    tmp_path = pdf_path.with_suffix(".pdf.v020d11.tmp")
...
def copy_outputs_to_patient_dir(
    ...
    for src in generated_paths:
        ...
        if doc_type == "CDPAS":
            ...
            stamp_cdpas_pdf_logo_only(dest.with_suffix(".pdf"))
...
def run_generator(
    ...
    if not generated_paths:
        raise RuntimeError(
            "No generated output paths were parsed from generate_outputs.py stdout. "
            "STDOUT was:\n" + stdout_text + "\n\nSTDERR was:\n" + stderr_text
        )
    ...
    if not success:
        raise RuntimeError(
            "Final output copy/post-processing did not complete. "
            f"Required={required}; Copied={sorted(copied.keys())}; "
            f"Parsed generated paths={[str(p) for p in generated_paths]}"
        )
